...
class Sensor:
    def __init__(self, id: int, measurement_source_id: int, type_sensor: str, name: str, unit: int, description: str):
...
class Model:
    def __init__(self, model_path: str, params: Dict = None, version: str = None, property_names: List[str] = None):
...
class RandomForestModel(Model):
    def __init__(self, model_path: str, params: Dict, version: str, property_names: List[str]):
...
class Block:
    def __init__(self, model: Model, sensors: Union[List[Sensor], List[Camera]], logger: logging.Logger):
        self.model = model
    ...
    def poll_sensors(self, url) -> Union[List[float], int, Dict[int, Image.Image]]:
        if isinstance(self.sensors[0], Sensor):  # Если просто датчик
            ...
            try:
                response = requests.post(url=f'http://{url}/measurements', json={'measurements': measurements})
                if response.ok:
...
class Handler:
    ...
    def add_block(self, model: Model, sensors: Union[List[Sensor], List[Camera]]) -> int:
