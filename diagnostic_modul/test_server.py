...
for idx, filename in enumerate(os.listdir("./cameras")):
    if filename.lower().endswith((".mp4", ".mov", ".avi")):
        cameras.append(
            {
                "ip": os.path.abspath(f"./cameras/{filename}"),
                "id": idx,
...
def set_value(camera_id):
    if request.headers["Content-Type"] == "application/json":
        ...
        return (
            jsonify(
                {"message": f"Data received for camera {camera_id}. Value: {value}"}
            ),
            200,
        )
    else:
...
def archivate(camera_id):
    ...
    image_file.save(
        r"C:\Users\boiko.k.v\Desktop\Carbon-Digital-Twin\diagnostic_modul\images-save"
        + image_file.filename
    )
...
measurement_sources = [
    {"id": 0, "name": "universal 1", "unit": 1},
    {"id": 1, "name": "universal 2", "unit": 2},
]
units = [
    [
        {
...
def process_measurement_data():
    ...
    for measurement in measurements:
        ...
        time_from = datetime.datetime.strptime(
            measurement.get("time_from"), "%Y-%m-%dT%H:%M:%SZ"
        )
        time_to = datetime.datetime.strptime(
            measurement.get("time_to"), "%Y-%m-%dT%H:%M:%SZ"
        )

        data_for_measurement = get_data_source(
            sensor_id, measurement_source_id, time_from, time_to
        )
...
def save_data_prediction():
    ...
    print(
        f"Получены следуещие данные: источники({sources}), названия свойств({property_names}), значения({values})"
    )
