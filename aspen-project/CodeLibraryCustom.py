...
class Simulation:
    ...
    AspenSimulation = win32.Dispatch("Apwn.Document")

    def __init__(
        self, AspenFileName: str, WorkingDirectoryPath: str, VISIBILITY: bool = True
    ):
    ...
    def StreamPlace(
        self, Streamname: str, Streamtype: Literal["MATERIAL", "HEAT", ""]
    ) -> None:  # Stream types are: "MATERIAL", "HEAT" or ""
    ...
    def StreamConnect(
        self, Blockname: str, Streamname: str, Portname: str
    ) -> None:  # Portnames for destillation column is: "D(OUT)" , "B(OUT)", "F(IN)"
    ...
    ###DSTWU
    def BLK_DSTWU_GET_ME_ALL_INPUTS_BACK(
        self, Blockname: str
        ...
        StageRefluxOption = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("OPT_NTRR").Value
        )
        NumberOfStages = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("NSTAGE").Value
        )
        RefluxRatio = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("RR").Value
        )
        CondenserPressure = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("PTOP").Value
        )
        ReboilerPressure = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("PBOT").Value
        )
        LightkeyComponent = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("LIGHTKEY").Value
        )
        HeavykeyComponent = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("HEAVYKEY").Value
        )
        LightkeyRecovery = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("RECOVL").Value
        )
        HeavykeyRecovery = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("RECOVH").Value
        )
        CondenserOption = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("OPT_RDV").Value
        )
        DestillVaporFraction = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("RDV").Value
        )
        GenerateTableOption = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("PLOT").Value
        )
        GenerateTable_FirstStage = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("LOWER").Value
        )
        GenerateTable_LastStage = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("UPPER").Value
        )
        GenerateTable_StageNumber = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("NPOINT").Value
        )
        CalculateHeightequivalentHETP_Option = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("OPT_CALHETP").Value
        )
        CalculateHeightequivalentHETP_PackedHeight = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("PACK_HEIGHT").Value
        )
        FreewaterOption = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("BLKOPFREWAT").Value
        )
        MaxNumberFlashIterations = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("MAXIT").Value
        )
        FlashConvergenceTolerance = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("FLASH_TOL").Value
        )
        MaxNumberMinStageIterations = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("MAXIT").Value
        )
        KvalueTolerance = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("K_TOL").Value
        )
        ProductTempTolerance = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("TEMP_TOL").Value
        )
        Dictionary = {
    ...
    def BLK_DSTWU_SET_ALL_INPUTS(
        self, Blockname: str, Dictionary: Dict[str, Union[str, float, int]]
    ) -> None:
        ...
        try:
            self.BLK.Elements(Blockname).Elements("Input").Elements(
                "OPT_NTRR"
            ).Value = Dictionary.get("StageRefluxOption")
            try:
                self.BLK.Elements(Blockname).Elements("Input").Elements(
                    "NSTAGE"
                ).Value = Dictionary.get("NumberOfStages")
            except Exception:
                pass
            try:
                self.BLK.Elements(Blockname).Elements("Input").Elements("RR").Value = (
                    Dictionary.get("RefluxRatio")
                )
            except Exception:
                pass
            self.BLK.Elements(Blockname).Elements("Input").Elements("PTOP").Value = (
                Dictionary.get("CondenserPressure")
            )
            self.BLK.Elements(Blockname).Elements("Input").Elements("PBOT").Value = (
                Dictionary.get("ReboilerPressure")
            )
            self.BLK.Elements(Blockname).Elements("Input").Elements(
                "LIGHTKEY"
            ).Value = Dictionary.get("LightkeyComponent")
            self.BLK.Elements(Blockname).Elements("Input").Elements(
                "HEAVYKEY"
            ).Value = Dictionary.get("HeavykeyComponent")
            self.BLK.Elements(Blockname).Elements("Input").Elements("RECOVL").Value = (
                Dictionary.get("LightkeyRecovery")
            )
            self.BLK.Elements(Blockname).Elements("Input").Elements("RECOVH").Value = (
                Dictionary.get("HeavykeyRecovery")
            )
            self.BLK.Elements(Blockname).Elements("Input").Elements("OPT_RDV").Value = (
                Dictionary.get("CondenserOption")
            )
            self.BLK.Elements(Blockname).Elements("Input").Elements("RDV").Value = (
                Dictionary.get("DestillVaporFraction")
            )
            self.BLK.Elements(Blockname).Elements("Input").Elements("PLOT").Value = (
                Dictionary.get("GenerateTableOption")
            )
            try:
                self.BLK.Elements(Blockname).Elements("Input").Elements(
                    "LOWER"
                ).Value = Dictionary.get("GenerateTable_FirstStage")
                self.BLK.Elements(Blockname).Elements("Input").Elements(
                    "UPPER"
                ).Value = Dictionary.get("GenerateTable_LastStage")
            except Exception:
                pass
            self.BLK.Elements(Blockname).Elements("Input").Elements("NPOINT").Value = (
                Dictionary.get("GenerateTable_StageNumber")
            )
            self.BLK.Elements(Blockname).Elements("Input").Elements(
                "OPT_CALHETP"
            ).Value = Dictionary.get("CalculateHeightequivalentHETP_Option")
            try:
                self.BLK.Elements(Blockname).Elements("Input").Elements(
                    "PACK_HEIGHT"
                ).Value = Dictionary.get("CalculateHeightequivalentHETP_PackedHeight")
            except Exception:
                pass
            self.BLK.Elements(Blockname).Elements("Input").Elements(
                "BLKOPFREWAT"
            ).Value = Dictionary.get("FreewaterOption")
            self.BLK.Elements(Blockname).Elements("Input").Elements(
                "FLASH_MAXIT"
            ).Value = Dictionary.get("MaxNumberFlashIterations")
            self.BLK.Elements(Blockname).Elements("Input").Elements(
                "FLASH_TOL"
            ).Value = Dictionary.get("FlashConvergenceTolerance")
            self.BLK.Elements(Blockname).Elements("Input").Elements("MAXIT").Value = (
                Dictionary.get("MaxNumberMinStageIterations")
            )
            self.BLK.Elements(Blockname).Elements("Input").Elements("K_TOL").Value = (
                Dictionary.get("KvalueTolerance")
            )
            self.BLK.Elements(Blockname).Elements("Input").Elements(
                "TEMP_TOL"
            ).Value = Dictionary.get("ProductTempTolerance")
        except Exception:
            pass
    ...
    def BLK_DSTWU_Set_StageRefluxOption(
        self, Blockname: str, StageRefluxOption: Literal["NSTAGE", "RR"]
    ) -> None:  # you can chose NSTAGE or RR
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "OPT_NTRR"
        ).Value = StageRefluxOption
        # if you chose: NSTAGE
    ...
    def BLK_DSTWU_Set_CondenserOption(
        self, Blockname: str, CondenserOption: Literal["LIQUID", "VAPOR", "VAPLIQ"]
    ):  # LIQUID VAPOR or VAPLIQ
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "OPT_RDV"
        ).Value = CondenserOption
        # if you chose: LIQUID or VAPOR:
        # you dont need to add anything

        # if you chose: VAPLIQ:

    def BLK_DSTWU_Set_VAPLIQ_DestillVaporFraction(
        self, Blockname, DestillVaporFraction
    ):
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "RDV"
        ).Value = DestillVaporFraction

    # PAGE 2         Calculation Options:
    def BLK_DSTWU_Set_GenerateTableOption(
        self, Blockname: str, GenerateTableOption: Literal["YES", "NO"]
    ) -> None:  # YES or NO
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "PLOT"
        ).Value = GenerateTableOption
    ...
    def BLK_DSTWU_Set_CalculateHeightequivalentHETP_Option(
        self, Blockname: str, CalculateHeightequivalentHETP_Option: Literal["YES", "NO"]
    ) -> None:  # YES or NO
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "OPT_CALHETP"
        ).Value = CalculateHeightequivalentHETP_Option
        # if you chose YES then you need to input this:

    def BLK_DSTWU_Set_CalculateHeightequivalentHETP_PackedHeight(
        self, Blockname: str, PackedHeight: Literal["YES", "NO"]
    ) -> None:  # YES or NO
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "PACK_HEIGHT"
        ).Value = PackedHeight
    ...
    # PAGE 3         Convergence:
    def BLK_DSTWU_Set_FreewaterOption(
        self, Blockname: str, FreewaterOption: Literal["YES", "NO", "DIRTY"]
    ) -> None:  # This can be YES, NO, DIRTY
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "BLKOPFREWAT"
        ).Value = FreewaterOption

    def BLK_DSTWU_Set_MaxNumberFlashIterations(
        self, Blockname, MaxNumberFlashIterations
    ):
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "FLASH_MAXIT"
        ).Value = MaxNumberFlashIterations

    def BLK_DSTWU_Set_FlashConvergenceTolerance(
        self, Blockname, FlashConvergenceTolerance
    ):
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "FLASH_TOL"
        ).Value = FlashConvergenceTolerance

    def BLK_DSTWU_Set_MaxNumberMinStageIterations(
        self, Blockname, MaxNumberMinStageIterations
    ):
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "MAXIT"
        ).Value = MaxNumberMinStageIterations

    def BLK_DSTWU_Set_KvalueTolerance(self, Blockname, KvalueTolerance):
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "K_TOL"
        ).Value = KvalueTolerance

    def BLK_DSTWU_Set_ProductTempTolerance(self, Blockname, ProductTempTolerance):
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "TEMP_TOL"
        ).Value = ProductTempTolerance

    def BLK_MIXER_GET_ME_ALL_INPUTS_BACK(
        self, Blockname: str
    ) -> Dict[str, Union[str, float, int]]:
        """Retrieves all the Inputs and returns Dictionary with Values
        ...
        Phase = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("Phase").Value
        )  # This can be V L or S
        Nphase = self.BLK.Elements(Blockname).Elements("Input").Elements("NPhase").Value
        TemperatureEstimation = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("T_EST").Value
        )
        MaximumIterations = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("MAXIT").Value
        )
        ErrorTolerance = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("TOL").Value
        )
    ...
    def BLK_MIXER_SET_ALL_INPUTS(
        ...
        self.BLK.Elements(Blockname).Elements("Input").Elements("PRES").Value = (
            Dictionary.get("Pressure")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("Phase").Value = (
            Dictionary.get("Phase")
        )  # This can be V L or S
        self.BLK.Elements(Blockname).Elements("Input").Elements("NPhase").Value = (
            Dictionary.get("Nphase")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("T_EST").Value = (
            Dictionary.get("TemperatureEstimate")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("MAXIT").Value = (
            Dictionary.get("MaximumIteration")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("TOL").Value = (
            Dictionary.get("ErrorTolerance")
        )
    ...
    def BLK_MIXER_Set_Phases(self, Blockname: str, Phase: Ph, Phasenumber: Phnum):
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "Phase"
        ).Value = Phase  # This can be V L or S
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "NPhase"
        ).Value = Phasenumber

    def BLK_MIXER_Set_TemperatureEstimate(
        self, Blockname: str, TempEstimate: float
    ):  # OPTIONAL
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "T_EST"
        ).Value = TempEstimate

    def BLK_MIXER_Set_MaximumIteration(
        self, Blockname: str, MaximumIteration: int
    ):  # OPTIONAL
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "MAXIT"
        ).Value = MaximumIteration

    def BLK_MIXER_Set_ErrorTolerance(
        self, Blockname: str, ErrorTolerance: float
    ):  # OPTIONAL
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "TOL"
        ).Value = ErrorTolerance

    def BLK_HEATER_GET_ME_ALL_INPUTS_BACK(
        self, Blockname: str
    ) -> Dict[str, Union[str, float, int]]:
        """Retrieves all the Inputs and returns Dictionary with Values
        ...
        """
        FlashTypeOption = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("SPEC_OPT").Value
        )
        Temperature = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("TEMP").Value
        )
        TemperatureChange = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("DELT").Value
        )
        DegreesSuperheating = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("DEGSUP").Value
        )
        DegreesSubcooling = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("DEGSUB").Value
        )
        Pressure = self.BLK.Elements(Blockname).Elements("Input").Elements("PRES").Value
        Duty = self.BLK.Elements(Blockname).Elements("Input").Elements("DUTY").Value
        Vaporfraction = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("VFRAC").Value
        )
        PressureDropCorrelation = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("DPPARM").Value
        )
        Phase = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("Phase").Value
        )  # This can be V L or S
        ...
        PressureEstimation = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("P_EST").Value
        )
        MaximumIteration = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("MAXIT").Value
        )
    ...
    def BLK_HEATER_SET_ALL_INPUTS(
        ...
        self.BLK.Elements(Blockname).Elements("Input").Elements("SPEC_OPT").Value = (
            Dictionary.get("FlashTypeOption")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("TEMP").Value = (
            Dictionary.get("Temperature")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("DELT").Value = (
            Dictionary.get("TemperatureChange")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("DEGSUP").Value = (
            Dictionary.get("DegreesSuperheating")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("DEGSUB").Value = (
            Dictionary.get("DegreesSubcooling")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("PRES").Value = (
            Dictionary.get("Pressure")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("DUTY").Value = (
            Dictionary.get("Duty")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("VFRAC").Value = (
            Dictionary.get("Vaporfraction")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("DPPARM").Value = (
            Dictionary.get("PressureDropCorrelation")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("Phase").Value = (
            Dictionary.get("Phase")
        )  # This can be V L or S
        self.BLK.Elements(Blockname).Elements("Input").Elements("NPhase").Value = (
            Dictionary.get("Phasenumber")
        )  # This can be 1,2,3
        self.BLK.Elements(Blockname).Elements("Input").Elements("T_EST").Value = (
            Dictionary.get("TemperatureEstimation")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("P_EST").Value = (
            Dictionary.get("PressureEstimation")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("MAXIT").Value = (
            Dictionary.get("MaximumIteration")
        )
    ...
    def BLK_HEATER_Set_FlashTypeOption(
        self,
        Blockname: str,
        FlashTypeOption: Literal[
            "TP",
            "TD",
            "TV",
            "TDPPARM",
        ...
        None
    ):  # You can chose between: TP, TD, TV, TDPPARM, PD, PV, PDT,PDEGSUP, PDEGSUB, DDPPARM, VDPPARM, DEGSUPDPPARM, DEGSUBDPPARM, DTV, DTD, DTDPPARM
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "SPEC_OPT"
        ).Value = FlashTypeOption
    ...
    def BLK_HEATER_Set_PressureDropCorrelation(
        self, Blockname, PressureDropCorrelation
    ):
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "DPPARM"
        ).Value = PressureDropCorrelation

    def BLK_HEATER_Set_Phases(
        self, Blockname: str, Phase: Ph, Phasenumber: Phnum
    ) -> None:
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "Phase"
        ).Value = Phase  # This can be V L or S
    ...
    def BLK_CISTR_GET_ME_ALL_INPUTS_BACK(
        self, Blockname: str
    ):  # -> Dict[str, Union[str,float,int]]
        """Retrieves all the Inputs and returns Dictionary with Values
        ...
        Specification_type = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("SPEC_TYPE").Value
        )  # This selects what input is needed:
        VolumeReactor = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("VOL").Value
        )
        ResidencetimeReactor = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("RES_TIME").Value
        )
        Specification_PhaseHoldup = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("SPEC_PHASE").Value
        )
        VolumeFrac_of_PhaseHoldup = (
            self.BLK.Elements(Blockname)
            .Elements("Input")
            .Elements("REACT_VOL_FR")
            .Value
        )
        Volume_of_PhaseHoldup = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("REACT_VOL").Value
        )
        Residencetime_of_PhaseHoldup = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("PH_RES_TIME").Value
        )

        StreamnameNode = (
            self.BLK.Elements(Blockname).Elements("Ports").Elements("F(IN)").Elements
        )

        ActivateReactions_or_not = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("REACSYS").Value
        )
        ActivateCrystalization_or_not = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("CRYSTSYS").Value
        )
        ActivateAgitation_or_not = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("AGITATOR").Value
        )
        AgitatorRotationrate = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("AGITRATE").Value
        )
        AgitatorImpellerDiameter = (
            self.BLK.Elements(Blockname)
            .Elements("Input")
            .Elements("IMPELLR_DIAM")
            .Value
        )
        AgitatorPowernumber = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("POWERNUMBER").Value
        )

        PSDCalculation_Option = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("OPT_PSD").Value
        )
        PSDParticalGrowthModel = (
            self.BLK.Elements(Blockname)
            .Elements("Input")
            .Elements("CONST_METHOD")
            .Value
        )

        CatalystPresentOption = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("CAT_PRESENT").Value
        )
        IgnoreCatalystVolume = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("IGN_CAT_VOL").Value
        )
        WeightOfCatalystLoaded = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("CATWT").Value
        )
        ParticleDensity = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("CAT_RHO").Value
        )
        BedVoidage = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("BED_VOIDAGE").Value
        )
    ...
    def BLK_CISTR_SET_ALL_INPUTS(
        ...
        self.BLK.ELements(Blockname).Elements("Input").Elements("SPEC_OPT").Value = (
            Dictionary.get("SpecificationOption")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("PRES").Value = (
            Dictionary.get("Pressure")
        )
        ...
        self.BLK.Elements(Blockname).Elements("Input").Elements("SPEC_TYPE").Value = (
            Dictionary.get("Specification_type")
        )  # This selects what input is needed:
        self.BLK.Elements(Blockname).Elements("Input").Elements("VOL").Value = (
            Dictionary.get("VolumeReactor")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("RES_TIME").Value = (
            Dictionary.get("ResidencetimeReactor")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("SPEC_PHASE").Value = (
            Dictionary.get("Specification_Phase")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "REACT_VOL_FR"
        ).Value = Dictionary.get("VolumeFrac_of_Phase")
        self.BLK.Elements(Blockname).Elements("Input").Elements("REACT_VOL").Value = (
            Dictionary.get("Volume_of_Phase")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("PH_RES_TIME").Value = (
            Dictionary.get("Residencetime_of_Holdup")
        )
        ...
        for Streamname in StreamnameNode:
            self.BLK.Elements(Blockname).Elements("Input").Elements(
                "PROD_PHASE"
            ).Elements(Streamname).Value = Dictionary.get("Streamphase")

        self.BLK.Elements(Blockname).Elements("Input").Elements("REACSYS").Value = (
            Dictionary.get("ActivateReactions_or_not")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("CRYSTSYS").Value = (
            Dictionary.get("ActivateCrystalization_or_not")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("AGITATOR").Value = (
            Dictionary.get("ActivateAgitation_or_not")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("AGITRATE").Value = (
            Dictionary.get("Rotationrate")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "IMPELLR_DIAM"
        ).Value = Dictionary.get("ImpellerDiameter")
        self.BLK.Elements(Blockname).Elements("Input").Elements("POWERNUMBER").Value = (
            Dictionary.get("Powernumber")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("OPT_PSD").Value = (
            Dictionary.get("CalculationOption")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "CONST_METHOD"
        ).Value = Dictionary.get("ParticalGrowthModel")
        self.BLK.Elements(Blockname).Elements("Input").Elements("CAT_PRESENT").Value = (
            Dictionary.get("CatalystPresentOption")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("IGN_CAT_VOL").Value = (
            Dictionary.get("IgnoreCatalystVolume")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("CATWT").Value = (
            Dictionary.get("WeightOfCatalystLoaded")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("CAT_RHO").Value = (
            Dictionary.get("ParticleDensity")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("BED_VOIDAGE").Value = (
            Dictionary.get("BedVoidage")
        )
    ...
    def BLK_CISTR_Set_Phases(
        self, Blockname: str, Phase: Ph, Phasenumber: Phnum
    ) -> None:
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "PHASE"
        ).Value = Phase  # This can be V L
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "NPHASE"
        ).Value = Phasenumber  # This can be 1,2,3

    def BLK_CISTR_Set_Specification_type(
        self,
        Blockname: str,
        Specification_type: Literal[
            ...
            "TOT-VOL-PH-VOL-FRAC",
            "TOT-VOL-PH-RES-TIME",
            "RES-TIME-PH-VOL-FRAC",
        ],
    ) -> None:
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "SPEC_TYPE"
        ).Value = Specification_type  # This selects what input is needed:
    ...
    def BLK_CISTR_Set_ResidenceTime(self, Blockname, ResidencetimeReactor):
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "RES_TIME"
        ).Value = ResidencetimeReactor

    def BLK_CISTR_Set_Specification_PhaseHoldup(self, Blockname, Specification_Phase):
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "SPEC_PHASE"
        ).Value = Specification_Phase

    def BLK_CISTR_Set_VolumeFrac_of_PhaseHoldup(self, Blockname, VolumeFrac_of_Phase):
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "REACT_VOL_FR"
        ).Value = VolumeFrac_of_Phase

    def BLK_CISTR_Set_Volume_of_PhaseHoldup(self, Blockname, Volume_of_Phase):
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "REACT_VOL"
        ).Value = Volume_of_Phase

    def BLK_CISTR_Set_Residencetime_of_PhaseHoldup(
        self, Blockname, Residencetime_of_Holdup
    ):
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "PH_RES_TIME"
        ).Value = Residencetime_of_Holdup

    ###PAGE 2       Streams
    def BLK_CISTR_Set_Productstream_phase(self, Blockname, Streamname, Streamphase):
        self.BLK.Elements(Blockname).Elements("Input").Elements("PROD_PHASE").Elements(
            Streamname
        ).Value = Streamphase

    ###PAGE 3       Kinetics
    def BLK_CISTR_Set_Activate_Reaction(
        self, Blockname: str, ActivateReactions_or_not: Literal["YES", "NO"]
    ):
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "REACSYS"
        ).Value = ActivateReactions_or_not

    ########    KINETICS IS STILL MISSING!!!!!!    #######
    def BLK_CISTR_Set_Activate_Crystalization(
        self, Blockname: str, ActivateCrystalization_or_not: Literal["YES", "NO"]
    ):
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "CRYSTSYS"
        ).Value = ActivateCrystalization_or_not

    def BLK_CISTR_Set_Activate_Agitation(
        self,
        Blockname: str,
        ActivateAgitation_or_not: Literal["YES", "NO"],
        Rotationrate: float,
        ...
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "AGITATOR"
        ).Value = ActivateAgitation_or_not
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "AGITRATE"
        ).Value = Rotationrate
        self.BLK.Elements(Blockname).Elements("Input").Elements(
    ...
    def BLK_CISTR_Set_Calculation_Option(
        self, Blockname: str, CalculationOption: Literal["COPY", "CONSTANT"]
    ) -> None:
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "OPT_PSD"
        ).Value = CalculationOption

    def BLK_CISTR_Set_ParticalGrowthModel(
        self,
        Blockname: str,
        ParticalGrowthModel: Literal[
            "DELTAD-NUM",
            "DELTAD-MASS",
            "DELTAV-NUM",
            "EQUI-MASS",
    ...
    ###PAGE 7       Catalysts
    def BLK_CISTR_Set_CatalystPresent(
        self, Blockname: str, CatalystPresentOption: Literal["YES", "NO"]
    ) -> None:
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "CAT_PRESENT"
        ).Value = CatalystPresentOption

    def BLK_CISTR_Set_IgnoreCatalystVolume(
        self, Blockname: str, IgnoreCatalystVolume: Literal["YES", "NO"]
    ) -> None:
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "IGN_CAT_VOL"
        ).Value = IgnoreCatalystVolume

    def BLK_CISTR_Set_WeightOfCatalystLoaded(self, Blockname, WeightOfCatalystLoaded):
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "CATWT"
        ).Value = WeightOfCatalystLoaded
    ...
    def BLK_RPLUG_GET_ME_ALL_INPUTS_BACK(
        self, Blockname: str
    ) -> Dict[str, Union[str, float, int]]:
        """Retrieves all the Inputs and returns Dictionary with Values
        ...
        Operating_conditions = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("OPT_TSPEC").Value
        )  # Chose between INLET-TEMP, CONST-TEMP, TEMP-PROF
        ReactorTemperature = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("REAC_TEMP").Value
        )

        Constant_Temp = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("CTEMP").Value
        )
        OutletTemp = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("TEMP").Value
        )
        ...
        Activate_YES_NO = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("CHK_NTUBE").Value
        )
        Number_of_Tubes = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("NTUBE").Value
        )
        TubeLength = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("LENGTH").Value
        )
        TubeDiameter = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("DIAM").Value
        )
        Phase = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("PHASE").Value
        )  # This can be V L or S
        ...
        ThermFluidPhase = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("CPHASE").Value
        )  # "V" or "L"
        ThermFluidPhaseNumber = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("CNPHASE").Value
        )  # 1 ,2 ,3

        StreaminPortList = (
            self.BLK.Elements(Blockname).Elements("Ports").Elements("P(OUT)").Elements
        )
        ...
        Streamphase = (
            ...
            .Elements("Input")
            .Elements("PROD_PHASE")
            .Elements(ListingOfStreamnamesinProductphase[0])
            .Value
        )
        ActivateReaction_YES_NO = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("REACSYS").Value
        )
        InletProcessflowPressure = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("PRES").Value
        )
        InletThermalfluidPressure = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("CPRES").Value
        )
        PressuredropCalulationOption = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("OPT_PDROP").Value
        )
        ThermalfluidPressureDrop = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("CPDROP").Value
        )
        ProcessflowPressureDrop = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("PDROP").Value
        )
        Roughnessvalue = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("ROUGHNESS").Value
        )
        PressuredropCorrelation = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("DP_FCOR").Value
        )
        CorrectionFactor = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("DP_MULT").Value
        )
        HoldupCalculationOption = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("OPT_HOLDUP").Value
        )
        HoldupCorrelation = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("DP_HCOR").Value
        )
        CatalystPresentOption = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("CAT_PRESENT").Value
        )
    ...
    def BLK_RPLUG_SET_ALL_INPUTS(
        ...
        self.BLK.Elements(Blockname).Elements("Input").Elements("TYPE").Value = (
            Dictionary.get("TYPE")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("OPT_TSPEC").Value = (
            Dictionary.get("Operating_conditions")
        )  # Chose between INLET-TEMP, CONST-TEMP, TEMP-PROF
        self.BLK.Elements(Blockname).Elements("Input").Elements("REAC_TEMP").Value = (
            Dictionary.get("ReactorTemperature")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("U").Value = (
            Dictionary.get("U")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("CTEMP").Value = (
            Dictionary.get("Constant_Temp")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("TEMP").Value = (
            Dictionary.get("OutletTemp")
        )
        # PAGE 2     General Reactor Config
        self.BLK.Elements(Blockname).Elements("Input").Elements("CHK_NTUBE").Value = (
            Dictionary.get("Activate_YES_NO")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("NTUBE").Value = (
            Dictionary.get("Number_of_Tubes")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("LENGTH").Value = (
            Dictionary.get("TubeLength")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("DIAM").Value = (
            Dictionary.get("TubeDiameter")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("PHASE").Value = (
            Dictionary.get("Phase")
        )  # This can be V L or S
        ...
        self.BLK.Elements(Blockname).Elements("Input").Elements("CPHASE").Value = (
            Dictionary.get("ThermFluidPhase")
        )  # "V" or "L"
        self.BLK.Elements(Blockname).Elements("Input").Elements("CNPHASE").Value = (
            Dictionary.get("ThermFluidPhaseNumber")
        )  # 1 ,2 ,3
        # PAGE 3     Streams
        StreaminPortList = (
            self.BLK.Elements(Blockname).Elements("Ports").Elements("P(OUT)").Elements
        ...
        self.BLK.Elements(Blockname).Elements("Input").Elements("PROD_PHASE").Elements(
            ListingOfStreamnamesinProductphase[0]
        ).Value = Dictionary.get("Streamphase")
        # PAGE 4     Reaction
        self.BLK.Elements(Blockname).Elements("Input").Elements("REACSYS").Value = (
            Dictionary.get("ActivateReaction_YES_NO")
        )
        # PAGE 5     Pressurespecification
        self.BLK.Elements(Blockname).Elements("Input").Elements("PRES").Value = (
            Dictionary.get("InletProcessflowPressure")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("CPRES").Value = (
            Dictionary.get("InletThermalfluidPressure")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("OPT_PDROP").Value = (
            Dictionary.get("PressuredropCalulationOption")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("CPDROP").Value = (
            Dictionary.get("ThermalfluidPressureDrop")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("PDROP").Value = (
            Dictionary.get("ProcessflowPressureDrop")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("ROUGHNESS").Value = (
            Dictionary.get("Roughnessvalue")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("CPDROP").Value = (
            Dictionary.get("ThermalfluidPressureDrop")
        )
        ...
        self.BLK.Elements(Blockname).Elements("Input").Elements("DP_MULT").Value = (
            Dictionary.get("CorrectionFactor")
        )
        # PAGE 6     Reactor holdup
        self.BLK.Elements(Blockname).Elements("Input").Elements("OPT_HOLDUP").Value = (
            Dictionary.get("HoldupCalculationOption")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("DP_HCOR").Value = (
            Dictionary.get("HoldupCorrelation")
        )
        # PAGE 7     CATALYST
        self.BLK.Elements(Blockname).Elements("Input").Elements("CAT_PRESENT").Value = (
            Dictionary.get("CatalystPresentOption")
    ...
    def BLK_RPLUG_Set_TYPE(
        self,
        Blockname: str,
        TYPE: Literal[
            "T-SPEC", "ADIABATIC", "TCOOL-SPEC", "CO-COOL", "TCOOL-PROF", "QFLUX-PROF"
        ],
    ) -> None:
    ...
    def BLK_RPLUG_Set_T_SPEC_Operating_condition(
        self,
        Blockname: str,
        Operating_conditions: Literal["INLET-TEMP", "CONST-TEMP", "TEMP-PROF"],
    ) -> None:
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "OPT_TSPEC"
        ).Value = (
            Operating_conditions  # Chose between INLET-TEMP, CONST-TEMP, TEMP-PROF
        )
    ...
    def BLK_RPLUG_Set_T_SPEC_TemperatureProfil(
        self, Blockname, TemperatureList, LocationList
    ):
        ...
        if len(TemperatureList) != len(LocationList):
            raise Exception(
                "TemperatureList and LocationList need to have the same length! PList:{} LList: {}".format(
        ...
        for Temp in TemperatureList:
            ...
            self.BLK.Elements(Blockname).Elements("Input").Elements(
                "SPEC_TEMP"
            ).Elements(listpositionname).Value = Temp
            i = i + 1
        i = 0
        for Location in LocationList:
            listpositionname = "#" + str(i)
            self.BLK.Elements(Blockname).Elements("Input").Elements("LOC").Elements(
                listpositionname
            ).Value = Location
            i = i + 1
        i = 0
    ...
    def BLK_RPLUG_Set_TCOOL_PROF_TemperatureProfil(
        self, Blockname, TemperatureList: list, LocationList: list
    ):
        """Sets the Temperature Profile in side of the Column
        ...
        for Temp in TemperatureList:
            listpositionname = "#" + str(i)
            self.BLK.Elements(Blockname).Elements("Input").Elements("TCOOL").Elements(
                listpositionname
            ).Value = Temp
            i = i + 1
        i = 0
        for Location in LocationList:
            ...
            self.BLK.Elements(Blockname).Elements("Input").Elements(
                "TCOOL_LOC"
            ).Elements(listpositionname).Value = Location
            i = i + 1
        i = 0

    # You chose QFLUX-PROF (defined Heatflux profile)
    def BLK_RPLUG_Set_QFLUX_PROF_HeatFluxProfil(
        self, Blockname: str, HeatFluxList: list, LocationList: list
    ) -> None:
        ...
        if len(HeatFluxList) != len(LocationList):
            raise Exception(
                "HeatFluxList and LocationList need to have the same length! PList:{} LList: {}".format(
                    len(HeatFluxList), len(LocationList)
                )
            )
        ...
        for HeatFlux in HeatFluxList:
            listpositionname = "#" + str(i)
            self.BLK.Elements(Blockname).Elements("Input").Elements("QFLUX").Elements(
                listpositionname
            ).Value = HeatFlux
            i = i + 1
        i = 0
    ...
    # PAGE 2     General Reactor Config
    def BLK_RPLUG_Set_Activate_Multitube_Reactor(
        self,
        Blockname: str,
        Activate_YES_NO: Literal["YES", "NO"],
        Number_of_Tubes: int,
    ):  # Optional
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "CHK_NTUBE"
        ).Value = Activate_YES_NO
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "NTUBE"
        ).Value = Number_of_Tubes
    ...
    def BLK_RPLUG_Set_Thermalfluid_ValidPhases(
        self,
        Blockname: str,
        ThermFluidPhase: Literal["V", "L"],
        ThermFluidPhaseNumber: Literal[1, 2, 3],
    ) -> None:
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "CPHASE"
        ).Value = ThermFluidPhase  # "V" or "L"
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "CNPHASE"
        ).Value = ThermFluidPhaseNumber  # 1 ,2 ,3
    ...
    # PAGE 4     Reaction
    def BLK_RPLUG_Set_ActivateReactions(
        self, Blockname: str, ActivateReaction_YES_NO: Literal["YES", "NO"]
    ) -> None:
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "REACSYS"
        ).Value = ActivateReaction_YES_NO

        ##MISSING THE MOVING THE REACTION THING OVER###

    def BLK_RPLUG_Set_ReactionActivities(
        self, Blockname: str, ActivityList: list, ActivityNameList: list
    ) -> None:
        ...
        if len(ActivityList) != len(ActivityNameList):
            raise Exception(
                "ActivityList and ActivityNameList need to have the same length! PList:{} LList: {}".format(
                    len(ActivityList), len(ActivityNameList)
                )
            )
        ...
        for Activity in ActivityList:
            ...
            self.BLK.Elements(Blockname).Elements("Input").Elements(
                "ACT_VALUE"
            ).Elements(listpositionname).Value = Activity
            i = i + 1
        i = 0
        for ActivityName in ActivityNameList:
            ...
            self.BLK.Elements(Blockname).Elements("Input").Elements(
                "???????????????"
            ).Elements(listpositionname).Value = ActivityName
            i = i + 1
        i = 0

    # PAGE 5     Pressurespecification
    def BLK_RPLUG_Set_InletProcessflowPressure(
        self, Blockname, InletProcessflowPressure
    ):
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "PRES"
        ).Value = InletProcessflowPressure

    def BLK_RPLUG_Set_InletThermalfluidPressure(
        self, Blockname, InletThermalfluidPressure
    ):
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "CPRES"
        ).Value = InletThermalfluidPressure
        # Chose Option for the Pressure drop calculation

    def BLK_RPLUG_Set_PressuredropCalulationOption(
        self,
        Blockname: str,
        PressuredropCalulationOption: Literal["SPECIFIED", "USER-SUBR", "CORRELATION"],
    ) -> None:  # Possibilites are: “SPECIFIED“, “USER-SUBR“, “CORRELATION“:
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "OPT_PDROP"
        ).Value = PressuredropCalulationOption

    # if you chose “SPECIFIED“
    def BLK_RPLUG_Set_SPECIFIED_ThermalfluidPressureDrop(
        self, Blockname, ThermalfluidPressureDrop
    ):
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "CPDROP"
        ).Value = ThermalfluidPressureDrop

    def BLK_RPLUG_Set_SPECIFIED_ProcessflowPressureDrop(
        self, Blockname, ProcessflowPressureDrop
    ):
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "PDROP"
        ).Value = ProcessflowPressureDrop
    ...
    def BLK_RPLUG_Set_CORRELATION_ThermalfluidPressureDrop(
        self, Blockname, ThermalfluidPressureDrop
    ):
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "CPDROP"
    ...
    def BLK_RPLUG_Set_CORRELATION_PressuredropCorrelation(
        self, Blockname, PressuredropCorrelation
    ):  # You can chose between: BEGGS-BRILL DUKLER SLACK ORKI AWR LOCK-MART H-BROWN DARCY ERGUN HTFS
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "DP_FCOR"
        ).Value = PressuredropCorrelation
    ...
    # PAGE 6     Reactor holdup
    def BLK_RPLUG_Set_HoldupCalculationOption(
        self,
        Blockname: str,
        HoldupCalculationOption: Literal["NO-SLIP", "CALCULATED", "SPECIFIED"],
    ):  # You can chose between NO-SLIP, CALCULATED, SPECIFIED
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "OPT_HOLDUP"
        ).Value = HoldupCalculationOption

    # if you chose NO-SLIP
    # Nothing is needed as Input

    # if you chose CALCULATED
    def BLK_RPLUG_Set_CALCULATED_HoldupCorrelation(
        self, Blockname, HoldupCorrelation
    ):  # You can chose between BEGGS-BRILL FLANIGAN EATON HOOG HUGH SLACK ORKI AWR LOCK-MART H-BROWN USER-SUBR HTFS
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "DP_HCOR"
        ).Value = HoldupCorrelation

    # if you chose SPECIFIED
    def BLK_RPLUG_Set_SPECIFIED_HoldupProfilSOLID(
        self, Blockname: str, HoldupList: list, LocationList: list
    ) -> None:
        ...
        if len(HoldupList) != len(LocationList):
            raise Exception(
                "HoldupList and LocationList need to have the same length! PList:{} LList: {}".format(
                    len(HoldupList), len(LocationList)
                )
            )
        ...
        for Holdup in HoldupList:
            listpositionname = "#" + str(i)
            self.BLK.Elements(Blockname).Elements("Input").Elements("SHOLDUP").Elements(
                listpositionname
            ).Value = Holdup
            i = i + 1
        i = 0
        for Location in LocationList:
            listpositionname = "#" + str(i)
            self.BLK.Elements(Blockname).Elements("Input").Elements("SHLOC").Elements(
                listpositionname
            ).Value = Location
            i = i + 1
        i = 0

    def BLK_RPLUG_Set_SPECIFIED_HoldupProfilLIQUID(
        self, Blockname: str, HoldupList: list, LocationList: list
    ):
        ...
        for Holdup in HoldupList:
            listpositionname = "#" + str(i)
            self.BLK.Elements(Blockname).Elements("Input").Elements("HOLDUP").Elements(
                listpositionname
            ).Value = Holdup
            i = i + 1
        i = 0
        for Location in LocationList:
            listpositionname = "#" + str(i)
            self.BLK.Elements(Blockname).Elements("Input").Elements("HLOCK").Elements(
                listpositionname
            ).Value = Location
            i = i + 1
        i = 0
    ...
    # PAGE 7     CATALYST
    def BLK_RPLUG_Set_CatalystPresent(
        self, Blockname: str, CatalystPresentOption: Literal["YES", "NO"]
    ):
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "CAT_PRESENT"
    ...
    def BLK_RPLUG_Set_IgnoreCatalystVolume(
        self, Blockname: str, IgnoreCatalystVolume: Literal["YES", "NO"]
    ):
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "IGN_CAT_VOL"
    ...
    def BLK_RADFRAC_GET_ME_ALL_INPUTS_BACK(
        self, Blockname: str
    ) -> Dict[str, Union[str, float, int]]:
        """Retrieves all the Inputs and returns Dictionary with Values
        ...
        CalculationType = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("CALC_MODE").Value
        )
        NStage = self.BLK.Elements(Blockname).Elements("Input").Elements("NSTAGE").Value
        CondenserType = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("CONDENSER").Value
        )
        ReboilerType = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("REBOILER").Value
        )
        Phase = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("Phase").Value
        )  # This can be V L or S
        ...
        ConvergenceMethod = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("CONV_METH").Value
        )
        Refluxratio = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("BASIS_RR").Value
        )
        Refluxrate = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("BASIS_L1").Value
        )
        BoilupRate = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("BASIS_VN").Value
        )
        BoilupRatio = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("BASIS_BR").Value
        )
        CondenserDuty = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("Q1").Value
        )
        ReboilerDuty = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("QN").Value
        )
        TotalDestillateFlowrate = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("BASIS_D").Value
        )
        LiquidBottomRate = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("BASIS_B").Value
        )
        DestillateToFeedRatio = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("BASIS_D:F").Value
        )
        BottomToFeedRatio = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("B:F").Value
        )
        # Page 2     Streams
        FeedStreamNameNode = (
            self.BLK.Elements(Blockname).Elements("Ports").Elements("F(IN)").Element
        )
        for FeedStreamName in FeedStreamNameNode:
            FeedStage = (
                ...
                .Elements("Input")
                .Elements("FEED_STAGE")
                .Elements(FeedStreamName)
                .Value
            )
            FeedStageLocation = (
                self.BLK.Elements(Blockname)
                .Elements("Input")
                .Elements("FEED_CONVE2")
                .Elements(FeedStreamName)
                .Value
            )

        CompleteProductStreamNameList = []
        ProductStreamNameList4LiquidDestillate = (
            self.BLK.Elements(Blockname).Elements("Ports").Elements("LD(OUT)").Element
        )
        for ProductStreamName in ProductStreamNameList4LiquidDestillate:
            CompleteProductStreamNameList.append(ProductStreamName)
        ProductStreamNameList4Bottoms = (
            self.BLK.Elements(Blockname).Elements("Ports").Elements("B(OUT)").Element
        )
        ...
        for ProductStreamName in CompleteProductStreamNameList:
            ...
            # Page 3     PRESSURE
        PressurePerspectiveOption = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("VIEW_PRES").Value
        )
        CondenserPressure = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("PRES1").Value
        )
        CondenserPressureDrop = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("PRES2").Value
        )
        StagePressureDrop = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("DP_STAGE").Value
        )
        # PAGE 4         Condenser
        CoolRefluxandDestillate = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("SC_OPTION").Value
        )
        CondenserTempOption = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("OPT_SUBCOOL").Value
        )
        SubcooledTemp = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("SC_TEMP").Value
        )
        DegreeSubcooled = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("DEGSUB").Value
        )
        CondenserOption = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("OPT_COND").Value
        )
        VaporTemp = self.BLK.Elements(Blockname).Elements("Input").Elements("T1").Value
        VaporFraction = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("BASIS_RDV").Value
        )
        ThermosyphonOption = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("OPT_TH_REB").Value
        )
        ReboilerCirculationFlow = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("TH_FLOW").Value
        )
        OutletTemperature = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("TH_TEMP").Value
        )
        ReboilerOutletPressure = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("TH_PRES").Value
        )
        ReboilerReturnLocation = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("RETURN_CONV").Value
        )
        ReboilerConfiguration = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("TSR_CONFIG").Value
        )
    ...
    def BLK_RADFRAC_SET_ALL_INPUTS(
        self, Blockname: str, Dictionary: Dict[str, Union[str, float, int]]
    ) -> None:
        """Takes Dictionary with Values set the ones which are given in Aspen.
        ...
        self.BLK.Elements(Blockname).Elements("Input").Elements("CALC_MODE").Value = (
            Dictionary.get("CalculationType")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("NSTAGE").Value = (
            Dictionary.get("NStage")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("CONDENSER").Value = (
            Dictionary.get("CondenserType")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("REBOILER").Value = (
            Dictionary.get("ReboilerType")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("Phase").Value = (
            Dictionary.get("Phase")
        )  # This can be V L or S
        ...
        self.BLK.Elements(Blockname).Elements("Input").Elements("CONV_METH").Value = (
            Dictionary.get("ConvergenceMethod")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("BASIS_RR").Value = (
            Dictionary.get("Refluxratio")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("BASIS_L1").Value = (
            Dictionary.get("Refluxrate")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("BASIS_VN").Value = (
            Dictionary.get("BoilupRate")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("BASIS_BR").Value = (
            Dictionary.get("BoilupRatio")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("Q1").Value = (
            Dictionary.get("CondenserDuty")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("QN").Value = (
            Dictionary.get("ReboilerDuty")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("BASIS_D").Value = (
            Dictionary.get("TotalDestillateFlowrate")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("BASIS_B").Value = (
            Dictionary.get("LiquidBottomRate")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("BASIS_D:F").Value = (
            Dictionary.get("DestillateToFeedRatio")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("BASIS_B:F").Value = (
            Dictionary.get("BottomToFeedRatio")
        )
        # Page 2     Streams
        ...
        for FeedStreamName in FeedStreamNameNode:
            self.BLK.Elements(Blockname).Elements("Input").Elements(
                "FEED_STAGE"
            ).Elements(FeedStreamName).Value = Dictionary.get("FeedStage")
            self.BLK.Elements(Blockname).Elements("Input").Elements(
                "FEED_CONVE2"
            ).Elements(FeedStreamName).Value = Dictionary.get("FeedStageLocation")
        ...
        for ProductStreamName in CompleteProductStreamNameList:
            self.BLK.Elements(Blockname).Elements("Input").Elements(
                "PROD_STAGE"
            ).Elements(ProductStreamName).Value = ProductStageLocationList[i]
            self.BLK.Elements(Blockname).Elements("Input").Elements(
                "PROD_PHASE"
            ).Elements(ProductStreamName).Value = ProductPhase[i]
            i = i + 1

        # Page 3     PRESSURE
        self.BLK.Elements(Blockname).Elements("Input").Elements("VIEW_PRES").Value = (
            Dictionary.get("PressurePerspectiveOption")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("PRES1").Value = (
            Dictionary.get("CondenserPressure")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("PRES2").Value = (
            Dictionary.get("CondenserPressureDrop")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("DP_STAGE").Value = (
            Dictionary.get("StagePressureDrop")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("PRES1").Value = (
            Dictionary.get("TopStagePressure")
        )
        # PAGE 4         Condenser
        self.BLK.Elements(Blockname).Elements("Input").Elements("OPT_SUBCOOL").Value = (
            Dictionary.get("CondenserTempOption")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("SC_TEMP").Value = (
            Dictionary.get("SubcooledTemp")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("DEGSUB").Value = (
            Dictionary.get("DegreeSubcooled")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("SC_OPTION").Value = (
            Dictionary.get("CoolRefluxandDestillate")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("OPT_SUBCOOL").Value = (
            Dictionary.get("CondenserTempOption")
        )
        ...
        self.BLK.Elements(Blockname).Elements("Input").Elements("T1").Value = (
            Dictionary.get("VaporTemp")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("BASIS_RDV").Value = (
            Dictionary.get("VaporFraction")
        )
        # PAGE 5 Reboiler
        self.BLK.Elements(Blockname).Elements("Input").Elements("OPT_TH_REB").Value = (
            Dictionary.get("ThermosyphonOption")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("TH_FLOW").Value = (
            Dictionary.get("ReboilerCirculationFlow")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("TH_TEMP").Value = (
            Dictionary.get("OutletTemperature")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("TH_FLOW").Value = (
            Dictionary.get("ReboilerCirculationFlow")
        )
        ...
        self.BLK.Elements(Blockname).Elements("Input").Elements("TH_PRES").Value = (
            Dictionary.get("ReboilerOutletPressure")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("RETURN_CONV").Value = (
            Dictionary.get("ReboilerReturnLocation")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("TSR_CONFIG").Value = (
            Dictionary.get("ReboilerConfiguration")
        )

    ##RADFRAC

    # PAGE 1         Configuration
    def BLK_RADFRAC_Set_CalculationType(
        self, Blockname: str, CalculationType: Literal["RIG-RATE", "EQUILIBRIUM"]
    ) -> None:  # This can be RIG-RATE,  EQUILIBRIUM
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "CALC_MODE"
    ...
    def BLK_RADFRAC_Set_CondenserType(
        self,
        Blockname: str,
        CondenserType: Literal["NONE", "TOTAL", "PARTIAL-V", "PARTIAL-V-L"],
    ) -> (
        None
    ):  # THIS can be NONE, TOTAL, PARTIAL-V, PARTIAL-V-L        Very important for Page 4
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "CONDENSER"
        ).Value = CondenserType

    def BLK_RADFRAC_Set_ReboilerType(
        self, Blockname: str, ReboilerType: Literal["NONE", "KETTLE", "THERMOSYPHON"]
    ):  # Can be NONE, KETTLE, THERMOSYPHON, This is important for Page 5
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "REBOILER"
        ).Value = ReboilerType
    ...
    def BLK_RADFRAC_Set_ConvergenceMethod(
        self,
        Blockname: str,
        ConvergenceMethod: Literal[
            "STANDARD", "PETROLEUM", "NONIDEAL", "AZEOTROPIC", "CRYOGENIX", "OTHERS"
        ],
    ) -> (
        None
    ):  # This can be STANDARD, PETROLEUM, NONIDEAL, AZEOTROPIC, CRYOGENIX, OTHERS
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "CONV_METH"
    ...
    def BLK_RADFRAC_Set_TotalDestillateFlowrate(
        self, Blockname, TotalDestillateFlowrate
    ):
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "BASIS_D"
        ).Value = TotalDestillateFlowrate

    def BLK_RADFRAC_Set_LiquidBottomRate(self, Blockname, LiquidBottomRate):
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "BASIS_B"
        ).Value = LiquidBottomRate

    def BLK_RADFRAC_Set_DestillateToFeedRatio(self, Blockname, DestillateToFeedRatio):
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "BASIS_D:F"
        ).Value = DestillateToFeedRatio

    def BLK_RADFRAC_Set_BottomToFeedRatio(self, Blockname, BottomToFeedRatio):
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "B:F"
        ).Value = BottomToFeedRatio

    # Page 2     Streams
    def BLK_RADFRAC_Set_FeedStage(self, Blockname, FeedStage, FeedstreamName):
        self.BLK.Elements(Blockname).Elements("Input").Elements("FEED_STAGE").Elements(
            FeedstreamName
        ).Value = FeedStage

    def BLK_RADFRAC_Set_FeedStageLocation(
        self,
        Blockname,
        FeedStageLocation: Literal[
            "ON-STAGE", "ABOVE-STAGE", "ON-STAGE-VAP", "ON-STAGE-LIQ"
        ],
        FeedstreamName: str,
    ) -> None:  # Location can be ON-STAGE, ABOVE-STAGE, ON-STAGE-VAP, ON-STAGE-LIQ
        self.BLK.Elements(Blockname).Elements("Input").Elements("FEED_CONVE2").Elements(
            FeedstreamName
        ).Value = FeedStageLocation

    def BLK_RADFRAC_Set_ProductStreamStage(
        self, Blockname, ProductStageLocation, ProductstreamName
    ):
        self.BLK.Elements(Blockname).Elements("Input").Elements("FEED_CONVE2").Elements(
            ProductstreamName
        ).Value = ProductStageLocation

    def BLK_RADFRAC_Set_ProductPhase(
        self,
        Blockname: str,
        ProductPhase: Literal["L", "L1", "L2", "W", "V", "TL", "TV"],
        ProductStreamName: str,
    ) -> None:  # ProductPhases: L, L1, L2, W, V, TL, TV
        self.BLK.Elements(Blockname).Elements("Input").Elements("PROD_PHASE").Elements(
            ProductStreamName
        ).Value = ProductPhase

    # Page 3     PRESSURE
    def BLK_RADFRAC_Set_PressurePerspectiveOption(
        self,
        Blockname: str,
        PressurePerspectiveOption: Literal["TOP/BOTTOM", "PROFILE", "PDROP"],
    ) -> None:  # You can chose TOP/BOTTOM, PROFILE, PDROP
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "VIEW_PRES"
        ).Value = PressurePerspectiveOption
    ...
    def BLK_RADFRAC_Set_TOPBOTTOM_CondenserPressureDrop(
        self, Blockname, CondenserPressureDrop
    ):
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "PRES2"
        ).Value = CondenserPressureDrop

    def BLK_RADFRAC_Set_TOPBOTTOM_StagePressureDrop(self, Blockname, StagePressureDrop):
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "DP_STAGE"
        ).Value = StagePressureDrop

    # if you chose PROFILE
    def BLK_RADFRAC_Set_PROFILE_Pressure(
        self, Blockname: str, PressureList: list, LocationList: list
    ) -> None:
        ...
        if len(PressureList) != len(LocationList):
            raise Exception(
                "PressureList and LocationList need to have the same length! PList:{} LList: {}".format(
                    len(PressureList), len(LocationList)
                )
            )
        ...
        for Pressure in PressureList:
            ...
            self.BLK.Elements(Blockname).Elements("Input").Elements(
                "STAGE_PRES"
            ).Elements(listpositionname).Value = Pressure
            i = i + 1
        i = 0
    ...
    def BLK_RADFRAC_Set_PDROP_TopStagePressure(self, Blockname, TopStagePressure):
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "PRES1"
        ).Value = TopStagePressure

    def BLK_RADFRAC_Set_PDROP_StagePDROP_Profile(
        self,
        Blockname: str,
        LocationList: list,
        StartingStageList: list,
        EndingStageList: list,
        PressureDropList: list,
    ) -> None:
        """Sets the Pressure Profile in side of the Column
        ...
        if len(PressureDropList) != len(LocationList):
            raise Exception(
                "PressureDropList and LocationList need to have the same length! PList:{} LList: {}".format(
                    len(PressureDropList), len(LocationList)
                )
            )
        if len(StartingStageList) != len(LocationList):
            raise Exception(
                "StartingStageList and LocationList need to have the same length! PList:{} LList: {}".format(
                    len(StartingStageList), len(LocationList)
                )
            )
        if len(EndingStageList) != len(LocationList):
            raise Exception(
                "EndingStageList and LocationList need to have the same length! PList:{} LList: {}".format(
                    len(EndingStageList), len(LocationList)
                )
            )
        ...
        for StartingStage in StartingStageList:
            ...
            self.BLK.Elements(Blockname).Elements("Input").Elements(
                "PRES_STAGE1"
            ).Elements(listpositionname).Value = StartingStage
            self.BLK.Elements(Blockname).Elements("Input").Elements(
                "PRES_STAGE2"
            ).Elements(listpositionname).Value = EndingStageList[i]
            self.BLK.Elements(Blockname).Elements("Input").Elements(
                "PDROP_SEC"
            ).Elements(listpositionname).Value = PressureDropList[i]
            i = i + 1
        i = 0
    ...
    def BLK_RADFRAC_Set_TOTALorPARTIALV_CondenserTempOption(
        self, Blockname: str, CondenserTempOption: Literal["TEMP", "SUBCOOL"]
    ) -> (
        None
    ):  # You can chose between TEMP for Subcooled temperature, or SUBCOOL for degrees subcooled
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "OPT_SUBCOOL"
        ).Value = CondenserTempOption
        # if you chose TEMP
    ...
    def BLK_RADFRAC_Set_TOTALorPARTIALV_SUBCOOL(self, Blockname, DegreeSubcooled):
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "DEGSUB"
        ).Value = DegreeSubcooled

    def BLK_RADFRAC_Set_TOTALorPARTIALV_CoolRefluxandDestillate(
        self,
        Blockname: str,
        CoolRefluxandDestillate: Literal["REFLUX-AND-DESTILLATE", "REFLUX-ONLY"],
    ) -> None:  # You can chose REFLUX-AND-DESTILLATE or REFLUX-ONLY
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "SC_OPTION"
        ).Value = CoolRefluxandDestillate

    # if PARTIAL_V_L
    def BLK_RADFRAC_Set_PARTIAL_V_L_CondenserTempOption(
        self, Blockname: str, CondenserTempOption: Literal["TEMP", "SUBCOOL"]
    ) -> (
        None
    ):  # You can chose between TEMP for Subcooled temperature, or SUBCOOL for degrees subcooled
    ...
    def BLK_RADFRAC_Set_PARTIAL_V_L_CoolRefluxandDestillate(
        self,
        Blockname: str,
        CoolRefluxandDestillate: Literal["REFLUX-AND-DESTILLATE", "REFLUX-ONLY"],
    ) -> None:  # You can chose REFLUX-AND-DESTILLATE or REFLUX-ONLY
    ...
    def BLK_RADFRAC_Set_PARTIAL_V_L_CondenserOption(
        self, Blockname: str, CondenserOption: Literal["TEMP", "VFRAC"]
    ) -> None:  # you can chose between TEMP and VFRAC
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "OPT_COND"
        ).Value = CondenserOption
        # if you chose TEMP
    ...
    def BLK_RADFRAC_Set_THERMOSYPHON_OPTIONS(
        self,
        Blockname: str,
        ThermosyphonOption: Literal["FLOW", "OUTLET", "FLOW+OUTLET"],
    ) -> None:  # You can chose between FLOW, OUTLET, FLOW+OUTLET
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "OPT_TH_REB"
        ).Value = ThermosyphonOption
        # if Flow was selected:

    def BLK_RADFRAC_Set_THERMOSYPHON_FLOW(self, Blockname, ReboilerCirculationFlow):
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "TH_FLOW"
        ).Value = ReboilerCirculationFlow
    ...
    def BLK_RADFRAC_Set_ReboilerOutletPressure(self, Blockname, ReboilerOutletPressure):
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "TH_PRES"
        ).Value = ReboilerOutletPressure

    def BLK_RADFRAC_Set_ReboilerReturnLocation(
        self, Blockname: str, ReboilerReturnLocation: Literal["ABOVE-STAGE", "ON-STAGE"]
    ) -> None:  # IT can be ABOVE-STAGE or ON-STAGE
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "RETURN_CONV"
        ).Value = ReboilerReturnLocation

    def BLK_RADFRAC_Set_ReboilerConfiguration(
        self, Blockname, ReboilerConfiguration: Literal[1, 2, 3]
    ):  # This can be 1 or 2 or 3.
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "TSR_CONFIG"
        ).Value = ReboilerConfiguration

    def BLK_FLASH2_GET_ME_ALL_INPUTS_BACK(
        self, Blockname: str
    ) -> Dict[str, Union[str, float, int]]:
        """Retrieves all the Inputs and returns Dictionary with Values
        ...
        Liquid_Entrainment = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("ENTRN").Value
        )
        Solid_Entrainment = (
            self.BLK.Elements(Blockname)
            .Elements("Input")
    ...
    def BLK_FLASH2_SET_ALL_INPUTS(
        ...
        # PAGE 3 ENTRAINMENT:
        self.BLK.Elements(Blockname).Elements("Input").Elements("ENTRN").Value = (
            Dictionary.get("Liquid_Entrainment")
        )
        self.BLK.Elements(Blockname).Elements("Input").Elements("VAPOR").Elements(
            "MIXED"
        ).Value = Dictionary.get("Solid_Entrainment")
        # PAGE 4 Particle Size Determination PSD:
        self.BLK.Elements(Blockname).Elements("Input").Elements("OPT_PSD").Value = (
            Dictionary.get("CalculationOption")
        )
    ...
    #### FLASH2
    # PAGE 1 Specification
    def BLK_FLASH2_Set_Flash_Type_Option(
        self,
        Blockname: str,
        FlashTypeOption: Literal["TP", "TD", "TV", "TQ", "PD", "PV", "PQ"],
    ) -> None:  # This can be TP,TD,TV,TQ,PD,PV,PQ
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "SPEC_OPT"
        ).Value = FlashTypeOption
    ...
    def BLK_FLASH2_Set_Liquid_Entrainment(self, Blockname, Liquid_Entrainment):
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "ENTRN"
        ).Value = Liquid_Entrainment

    def BLK_FLASH2_Set_Solid_Entrainment(self, Blockname, Solid_Entrainment):
        self.BLK.Elements(Blockname).Elements("Input").Elements("VAPOR").Elements(
            "MIXED"
        ).Value = Solid_Entrainment

    # PAGE 4 Particle Size Determination PSD:
    def BLK_FLASH2_Set_Calculation_Option(
        self, Blockname: str, CalculationOption: Literal["COPY", "CONSTANT"]
    ) -> None:  # This can be COPY or CONSTANT
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "OPT_PSD"
        ).Value = CalculationOption

    def BLK_FLASH2_Set_ParticalGrowthModel(
        self,
        Blockname,
        ParticalGrowthModel: Literal[
            "DELTAD-NUM",
        ...
        None
    ):  # if you chose Constant you chose model: DELTAD-NUM, DELTAD-MASS, DELTAV-NUM, EQUI-MASS, EQUI-SURFACE, EQUI_NUMBER
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "CONST_METHOD"
        ).Value = ParticalGrowthModel
        ## USER SPECIFIED PSD
    ...
    def BLK_SPLITTER_GET_ME_ALL_INPUTS_BACK(
        self, Blockname: str
    ) -> Dict[str, Union[str, float, int]]:
        """Retrieves all the Inputs and returns Dictionary with Values
        ...
        ProductStreamNameNode = (
            self.BLK.Elements(Blockname).Elements("Ports").Elements("P(OUT)").Element
        )
        ...
        for ProductStreamName in ProductStreamNameNode:
            ...
            LimitFlowList.append(
                self.BLK.Elements(Blockname)
                .Elements("Input")
                .Elements("BASIS_LIMIT")
                .Elements(ProductStreamName)
                .Value
            )
            VolumeLimitFlowList.append(
                self.BLK.Elements(Blockname)
                .Elements("Input")
                .Elements("VOL_LIMIT")
                .Elements(ProductStreamName)
            ...
            CumLimitFlowList.append(
                self.BLK.Elements(Blockname)
                .Elements("Input")
                .Elements("BASIS_C_LIM")
                .Elements(ProductStreamName)
                .Value
            )
            CumVolumeLimitFlowList.append(
                self.BLK.Elements(Blockname)
                .Elements("Input")
                .Elements("VOL_C_LIM")
                .Elements(ProductStreamName)
    ...
    def BLK_SPLITTER_SET_ALL_INPUTS(
        self, Blockname: str, Dictionary: Dict[str, Union[str, float, int]]
    ) -> None:
        """Takes Dictionary with Values set the ones which are given in Aspen.
        ...
        for (
            ...
            ProductStreamNameList
        ):  # You need to loop through all the ProductStreamNames
            self.BLK.Elements(Blockname).Elements("Input").Elements("FRAC").Elements(
                ProductStreamName
            ).Value = SplitFractionList[i]
            self.BLK.Elements(Blockname).Elements("Input").Elements(
                "BASIS_FLOW"
            ).Elements(ProductStreamName).Value = FlowList[i]
            self.BLK.Elements(Blockname).Elements("Input").Elements(
                "VOL_FLOW"
            ).Elements(ProductStreamName).Value = ActualVolumeFlowList[i]
            self.BLK.Elements(Blockname).Elements("Input").Elements(
                "BASIS_LIMIT"
            ).Elements(ProductStreamName).Value = LimitFlowList[i]
            self.BLK.Elements(Blockname).Elements("Input").Elements(
                "VOL_LIMIT"
            ).Elements(ProductStreamName).Value = VolumeLimitFlowList[i]
            self.BLK.Elements(Blockname).Elements("Input").Elements(
                "BASIS_C_LIM"
            ).Elements(ProductStreamName).Value = CumLimitFlowList[i]
            self.BLK.Elements(Blockname).Elements("Input").Elements(
                "VOL_C_LIM"
            ).Elements(ProductStreamName).Value = CumVolumeLimitFlowList[i]
            self.BLK.Elements(Blockname).Elements("Input").Elements("R_FRAC").Elements(
                ProductStreamName
            ).Value = ResidualFractionList[i]
            i = i + 1
        i = 0
    ...
    def BLK_SPLITTER_Set_By_Flow(self, Blockname, Streamname, Flow):
        self.BLK.Elements(Blockname).Elements("Input").Elements("BASIS_FLOW").Elements(
            Streamname
        ).Value = Flow

    def BLK_SPLITTER_Set_By_ActualVolumeFlow(
        self, Blockname, Streamname, ActualVolumeFlow
    ):
        self.BLK.Elements(Blockname).Elements("Input").Elements("VOL_FLOW").Elements(
            Streamname
        ).Value = ActualVolumeFlow

    def BLK_SPLITTER_Set_By_LimitFlow(self, Blockname, Streamname, LimitFlow):
        self.BLK.Elements(Blockname).Elements("Input").Elements("BASIS_LIMIT").Elements(
            Streamname
        ).Value = LimitFlow

    def BLK_SPLITTER_Set_By_VolumeLimitFlow(
        self, Blockname, Streamname, VolumeLimitFlow
    ):
        self.BLK.Elements(Blockname).Elements("Input").Elements("VOL_LIMIT").Elements(
            Streamname
        ).Value = VolumeLimitFlow

    def BLK_SPLITTER_Set_By_CumLimitFlow(self, Blockname, Streamname, CumLimitFlow):
        self.BLK.Elements(Blockname).Elements("Input").Elements("BASIS_C_LIM").Elements(
            Streamname
        ).Value = CumLimitFlow

    def BLK_SPLITTER_Set_By_CumVolumeLimitFlow(
        self, Blockname, Streamname, CumVolumeLimitFlow
    ):
        self.BLK.Elements(Blockname).Elements("Input").Elements("VOL_C_LIM").Elements(
            Streamname
        ).Value = CumVolumeLimitFlow

    def BLK_SPLITTER_Set_By_ResidualFraction(
        self, Blockname, Streamname, ResidualFraction
    ):
        self.BLK.Elements(Blockname).Elements("Input").Elements("R_FRAC").Elements(
            Streamname
    ...
    def BLK_SPLITTER_Set_MaximumIteration(
        self, Blockname, MaximumIteration
    ):  # OPTIONAL
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "MAXIT"
        ).Value = MaximumIteration
    ...
    def BLK_RYIELD_GET_ME_ALL_INPUTS_BACK(
        self, Blockname
    ) -> Dict[str, Union[str, float, int]]:
        """Retrieves all the Inputs and returns Dictionary with Values
        ...
        YieldCalcOption = (
            self.BLK.Elements(Blockname).Elements("Input").Elements("USER_YIELD").Value
        )
    ...
    def BLK_RYIELD_SET_ALL_INPUTS(
        ...
        for PhaseOfProductStreamname in PhaseOfProductStreamnameList:
            self.BLK.Elements(Blockname).Elements("Input").Elements(
                "PROD_PHASE"
            ).Elements(PhaseOfProductStreamname).Value = PhaseOfProductStreamList[i]
            i = i + 1
        i = 0
        # PAGE 3 Yields:
        self.BLK.Elements(Blockname).Elements("Input").Elements("USER_YIELD").Value = (
            Dictionary.get("YieldCalcOption")
        )
        ...
        for YieldPerFlowCompound in YieldPerFlowCompoundList:
            self.BLK.Elements(Blockname).Elements("Input").Elements(
                "MOLE_YIELD"
            ).Elements(YieldPerFlowCompound).Value = YieldPerFlowList[i]
            self.BLK.Elements(Blockname).Elements("Input").Elements("BASIS").Elements(
                YieldPerFlowCompound
            ).Value = NewBasisList[i]
            i = i + 1
        i = 0
        ...
        for InertNumber in InertNumberList:
            self.BLK.Elements(Blockname).Elements("Input").Elements(
                "COMP_LIST"
            ).Elements(InertNumber).Value = InertComponentList[i]
            i = i + 1
        i = 0
        # PAGE 4 FLASH OPTION:
        self.BLK.Elements(Blockname).Elements("Input").Elements("T_EST").Value = (
    ...
    # PAGE 1 Specification:
    def BLK_RYIELD_Set_FlashTypeOption(
        self,
        Blockname: str,
        FlashTypeOption: Literal[
            "TP", "TD", "TV", "PD", "PV", "DTV", "DTD", "DTP", "DTQ"
        ],
    ) -> None:  # You can chose between: TP, TD, TV, PD, PV, DTV, DTD, DTP, DTQ
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "SPEC_OPT"
        ).Value = FlashTypeOption
    ...
    # PAGE 2 Streams:
    def BLK_RYIELD_Set_PhaseOfProductStream(
        self,
        Blockname: str,
        PhaseOfProductStream: Literal[
            "V", "L", "L1", "L2", "W", "VL", "VL1", "LW", "L1L2"
        ],
        Streamname: str,
    ):  # This can be V, L, L1,L2,W,VL,VL1,LW,L1L2
        self.BLK.Elements(Blockname).Elements("Input").Elements("PROD_PHASE").Elements(
            Streamname
        ).Value = PhaseOfProductStream

    # PAGE 3 Yields:

    def BLK_RYIELD_Set_YieldCalcOption(
        self, Blockname: str, YieldCalcOption: Literal["NO", "YES", "NO2", "NO3"]
    ):  # This can be NO, YES, NO2, NO3
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "USER_YIELD"
        ).Value = YieldCalcOption
        # if you chose: NO (Component yields)

    def BLK_RYIELD_Set_ComponentYield_YieldPerFlow(
        self, Blockname, YieldPerFlow, CompoundName
    ):  # Compoundname should be "ETHAN-01 MIXED"
        self.BLK.Elements(Blockname).Elements("Input").Elements("MOLE_YIELD").Elements(
            CompoundName
        ).Value = YieldPerFlow

    def BLK_RYIELD_Set_ComponentYield_ChangeBasis(
        self, Blockname, CompoundName, NewBasis: Literal["MASS", "MOLE"]
    ) -> (
        None
    ):  # Compoundname should be like: "ETHAN-01 MIXED", BaseOptions are "MASS" , "MOLE"
        self.BLK.Elements(Blockname).Elements("Input").Elements("BASIS").Elements(
            CompoundName
        ).Value = NewBasis

    def BLK_RYIELD_Set_ComponentYield_InertComponent(
        self, Blockname, InertComponent, InertNumber
    ):  # InertNumber should be either #0 or 0 ??? not sure..        InertComponent could be WATER
        self.BLK.Elements(Blockname).Elements("Input").Elements("COMP_LIST").Elements(
            InertNumber
        ).Value = InertComponent
    ...
    # PAGE 5 Particle Size Determination, PSD
    def BLK_RYIELD_Set_Calculation_Option(
        self, Blockname: str, CalculationOption: Literal["COPY", "CONSTANT", "SPEC"]
    ) -> None:  # This can be COPY or CONSTANT or SPEC
        self.BLK.Elements(Blockname).Elements("Input").Elements(
            "OPT_PSD"
        ).Value = CalculationOption
    ...
    def BLK_RYIELD_Set_ParticalGrowthModel(
        self,
        Blockname,
        ParticalGrowthModel: Literal[
            "DELTAD-NUM",
    ...
    def STRM_GET_ME_ALL_INPUTS_BACK(
        self, Streamname: str
    ) -> Dict[str, Union[str, float, int]]:
        """Retrieves all the Inputs and returns Dictionary with Values
        ...
        TotalFlowRate = (
            self.STRM.Elements(Streamname).Elements("Input").Elements("TOTFLOW").Value
        )
        ...
        CalculateStreamPropertiesOption = (
            self.STRM.Elements(Streamname)
            .Elements("Input")
            .Elements("FL_OPTION")
            .Elements("MIXED")
        ...
        ComponentTolerance = (
            self.STRM.Elements(Streamname)
            .Elements("Input")
            .Elements("AUTO_COMPS_T")
            .Value
        )
        ChooseAdditionalOptions = (
            self.STRM.Elements(Streamname).Elements("Input").Elements("EO_COMPS").Value
        )
        SolutionMethod = (
            self.STRM.Elements(Streamname)
            .Elements("Input")
        ...
        OpenDerivationMethod = (
            self.STRM.Elements(Streamname)
            .Elements("Input")
            .Elements("DERIV_METHOD")
            .Value
        )
        PassthroughOption = (
            self.STRM.Elements(Streamname)
            .Elements("Input")
            .Elements("PASS_THROUGH")
            .Value
        )
        NegativeComponentCheckTol = (
            self.STRM.Elements(Streamname)
            .Elements("Input")
            .Elements("NEG_COMP_CHK")
            .Value
        )
        NegativeFlowCheckTol = (
            self.STRM.Elements(Streamname)
            .Elements("Input")
            .Elements("NEG_FLOW_CHK")
            .Value
        )
        AlwaysInstantiate = (
            self.STRM.Elements(Streamname).Elements("Input").Elements("").Value
        )
        Sparcity = (
            self.STRM.Elements(Streamname).Elements("Input").Elements("SPARCITY").Value
        )
        Lightkey = (
            self.STRM.Elements(Streamname)
            .Elements("Input")
            .Elements("EO_LIGHT_KEY")
            .Value
        )
        Heavykey = (
            self.STRM.Elements(Streamname)
            .Elements("Input")
            .Elements("EO_HEAVY_KEY")
            .Value
        )
        WaterOnlyCheck = (
            self.STRM.Elements(Streamname)
            .Elements("Input")
            .Elements("CHECK_FREE_W")
            .Value
        )
        AutoPhaseOption = (
            self.STRM.Elements(Streamname)
        ...
        VfracTol = (
            self.STRM.Elements(Streamname).Elements("Input").Elements("VFRAC_TOL").Value
        )
        SfracTol = (
            self.STRM.Elements(Streamname).Elements("Input").Elements("SFRAC_TOL").Value
        )
        CompositionTol = (
            self.STRM.Elements(Streamname).Elements("Input").Elements("COMP_TOL").Value
        )
        TemperatureTol = (
            self.STRM.Elements(Streamname)
            .Elements("Input")
            .Elements("EO_TEMP_TOL")
            .Value
        )
        PricePerUnit = (
            self.STRM.Elements(Streamname).Elements("Input").Elements("PRICE").Value
        )
        PriceUnit = (
            self.STRM.Elements(Streamname).Elements("Input").Elements("PRICE").Basis
        )
    ...
    def STRM_SET_ALL_INPUTS(
        self, Streamname: str, Dictionary: Dict[str, Union[str, float, int]]
    ) -> None:
        """Takes Dictionary with Values set the ones which are given in Aspen.
        ...
        self.STRM.Elements(Streamname).Elements("Input").Elements(
            "MIXED_SPEC"
        ).Elements("MIXED").Value = Dictionary.get("FlashtypeChoice")
        self.STRM.Elements(Streamname).Elements("Input").Elements("TEMP").Elements(
            "MIXED"
        ).Value = Dictionary.get("Temp")
        self.STRM.Elements(Streamname).Elements("Input").Elements("PRES").Elements(
            "MIXED"
        ).Value = Dictionary.get("Pressure")
        self.STRM.Elements(Streamname).Elements("Input").Elements("VFRAC").Elements(
            "MIXED"
        ).Value = Dictionary.get("VaporFraction")
        self.STRM.Elements(Streamname).Elements("Input").Elements("TOTFLOW").Value = (
            Dictionary.get("TotalFlowRate")
        )
        ...
        for Compoundname in CompoundNameList:
            self.STRM.Elements(Streamname).Elements("Input").Elements("FLOW").Elements(
                "MIXED"
            ).Elements(Compoundname).Value = TotalFlowBasisList[i]
            self.STRM.Elements(Streamname).Elements("Input").Elements("FLOW").Elements(
                "MIXED"
            ).Elements(Compoundname).Value = ComponentFlowRateList[i]
            i = i + 1
        i = 0
        # Page 2    CI Solid
        # PAGE 3     NC Solid
        # PAGE 4     Flash Option
        self.STRM.Elements(Streamname).Elements("Input").Elements("FL_OPTION").Elements(
            "MIXED"
        ).Value = Dictionary.get("CalculateStreamPropertiesOption")
        # PAGE 5 EO OPTIONS
        self.STRM.Elements(Streamname).Elements("Input").Elements(
            "AUTO_COMPS"
        ).Value = Dictionary.get("RemoveComponentOption")
        self.STRM.Elements(Streamname).Elements("Input").Elements(
            "AUTO_COMPS_T"
        ).Value = Dictionary.get("ComponentTolerance")
        self.STRM.Elements(Streamname).Elements("Input").Elements("EO_COMPS").Value = (
            Dictionary.get("ChooseAdditionalOptions")
        )
        self.STRM.Elements(Streamname).Elements("Input").Elements(
            "SOL_METHOD"
        ).Value = Dictionary.get("SolutionMethod")
        self.STRM.Elements(Streamname).Elements("Input").Elements(
            "DERIV_METHOD"
        ).Value = Dictionary.get("OpenDerivationMethod")
        self.STRM.Elements(Streamname).Elements("Input").Elements(
            "PASS_THROUGH"
        ).Value = Dictionary.get("YesOrNO")
        self.STRM.Elements(Streamname).Elements("Input").Elements(
            "NEG_COMP_CHK"
        ).Value = Dictionary.get("NegativeComponentCheckTol")
        self.STRM.Elements(Streamname).Elements("Input").Elements(
            "NEG_FLOW_CHK"
        ).Value = Dictionary.get("NegativeFlowCheckTol")
        self.STRM.Elements(Streamname).Elements("Input").Elements("").Value = (
            Dictionary.get("AlwaysInstantiate")
        )
        self.STRM.Elements(Streamname).Elements("Input").Elements("SPARCITY").Value = (
            Dictionary.get("Sparcity")
        )
        self.STRM.Elements(Streamname).Elements("Input").Elements(
            "EO_LIGHT_KEY"
        ).Value = Dictionary.get("Lightkey")
        self.STRM.Elements(Streamname).Elements("Input").Elements(
            "EO_HEAVY_KEY"
        ).Value = Dictionary.get("Heavykey")
        self.STRM.Elements(Streamname).Elements("Input").Elements(
            "CHECK_FREE_W"
        ).Value = Dictionary.get("WaterOnlyCheck")
        self.STRM.Elements(Streamname).Elements("Input").Elements(
            "AUTO_PHASE"
        ).Value = Dictionary.get("YesOrNo")
        self.STRM.Elements(Streamname).Elements("Input").Elements(
            "AUTO_PHASE_T"
        ).Value = Dictionary.get("PhaseTolerance")
        self.STRM.Elements(Streamname).Elements("Input").Elements(
            "FLASH_FORM"
        ).Value = Dictionary.get("FlashFormulation")
        self.STRM.Elements(Streamname).Elements("Input").Elements(
            "VFRACX_TOL"
        ).Value = Dictionary.get("VfracXTol")
        self.STRM.Elements(Streamname).Elements("Input").Elements("VFRAC_TOL").Value = (
            Dictionary.get("VfracTol")
        )
        self.STRM.Elements(Streamname).Elements("Input").Elements("SFRAC_TOL").Value = (
            Dictionary.get("SfracTol")
        )
        self.STRM.Elements(Streamname).Elements("Input").Elements("COMP_TOL").Value = (
            Dictionary.get("CompositionTol")
        )
        self.STRM.Elements(Streamname).Elements("Input").Elements(
            "EO_TEMP_TOL"
        ).Value = Dictionary.get("TemperatureTol")
        # PAGE 6   Costing
        self.STRM.Elements(Streamname).Elements("Input").Elements("PRICE").Value = (
            Dictionary.get("PricePerUnit")
        )
        self.STRM.Elements(Streamname).Elements("Input").Elements("PRICE").Basis = (
            Dictionary.get("PriceUnit")
        )

    #### INPUT FOR STREAMS

    # Specifications: PAGE 1
    def STRM_Set_FlashTypeOption(
        self, Streamname: str, FlashtypeChoice: Literal["TP", "TV", "PV"]
    ) -> None:  #   This choses the Inputs: can take TP, TV, PV
        self.STRM.Elements(Streamname).Elements("Input").Elements(
            "MIXED_SPEC"
        ).Elements("MIXED").Value = FlashtypeChoice
    ...
    def STRM_Set_TotalFlowBasis(self, Streamname, TotalFlowBasis, Compoundname):
        self.STRM.Elements(Streamname).Elements("Input").Elements("FLOW").Elements(
            "MIXED"
        ).Elements(Compoundname).Value = TotalFlowBasis

    def STRM_Set_ComponentFlowRate(self, Streamname, ComponentFlowRate, Compoundname):
        self.STRM.Elements(Streamname).Elements("Input").Elements("FLOW").Elements(
            "MIXED"
        ).Elements(Compoundname).Value = ComponentFlowRate
    ...
    # PAGE 4     Flash Option
    def STRM_Set_CalculateStreamPropertiesOption(
        self, Streamname: str, CalculateStreamPropertiesOption: Literal["NOFLASH", ""]
    ):  # This can be either "NOFLASH" or "" (nothing)
        self.STRM.Elements(Streamname).Elements("Input").Elements("FL_OPTION").Elements(
            "MIXED"
        ).Value = CalculateStreamPropertiesOption

    # PAGE 5 EO OPTIONS
    #   def STRM_ModelComponent(self, Streamname, ):
    #      self.STRM.Elements(Streamname).Elements("Input").Elements("").Elements("MIXED").Value =
    #                   I DONT KNOW HOW TO DO THIS
    def STRM_Set_RemoveComponentOption(
        self,
        Streamname: str,
        RemoveComponentOption: Literal["ALWAYS", "IF-NO-COMPS", "NEVER"],
    ) -> None:  # THIS can be ALWAYS, IF-NO-COMPS, NEVER
        self.STRM.Elements(Streamname).Elements("Input").Elements(
            "AUTO_COMPS"
        ).Value = RemoveComponentOption

    def STRM_Set_ComponentTolerance(self, Streamname, ComponentTolerance):
        self.STRM.Elements(Streamname).Elements("Input").Elements(
            "AUTO_COMPS_T"
        ).Value = ComponentTolerance

    def STRM_Set_ChooseAdditionalOptions(self, Streamname, ChooseAdditionalOptions):
        self.STRM.Elements(Streamname).Elements("Input").Elements(
            "EO_COMPS"
        ).Value = ChooseAdditionalOptions

        # ADDITIONAL OPTIONS:

    def STRM_Set_AddOptSolutionMethod(
        self,
        Streamname: str,
        SolutionMethod: Literal[
            "OPEN-PERT-IN", "OPEN-PERT-WA", "OPEN-NOPERT", "PERTUBATION", "DO-NOT-CREAT"
        ],
    ) -> (
        None
    ):  # Choices are:	OPEN-PERT-IN   OPEN-PERT-WA    OPEN-NOPERT    PERTUBATION   DO-NOT-CREAT
        self.STRM.Elements(Streamname).Elements("Input").Elements(
            "SOL_METHOD"
        ).Value = SolutionMethod

    def STRM_Set_AddOptOpenDerivationMethod(
        self,
        Streamname: str,
        OpenDerivationMethod: Literal[
            "ANALYTICAL", "NUMERICAL", "UPDATE-ANALY", "UPDATE-NUMER"
        ],
    ):  # ANALYTICAL, NUMERICAL, UPDATE-ANALY, UPDATE-NUMER
        self.STRM.Elements(Streamname).Elements("Input").Elements(
            "DERIV_METHOD"
        ).Value = OpenDerivationMethod

    def STRM_Set_AddOptPassThrough(
        self, Streamname: str, YesOrNO: Literal["YES", "NO"]
    ) -> None:
        self.STRM.Elements(Streamname).Elements("Input").Elements(
            "PASS_THROUGH"
        ).Value = YesOrNO

    def STRM_Set_AddOptNegativeComponentCheckTol(
        self, Streamname, NegativeComponentCheckTol
    ):
        self.STRM.Elements(Streamname).Elements("Input").Elements(
            "NEG_COMP_CHK"
        ).Value = NegativeComponentCheckTol

    def STRM_Set_AddOptNegativeFlowCheckTol(self, Streamname, NegativeFlowCheckTol):
        self.STRM.Elements(Streamname).Elements("Input").Elements(
            "NEG_FLOW_CHK"
        ).Value = NegativeFlowCheckTol

    def STRM_Set_AddOptAlwaysInstantiate(self, Streamname, AlwaysInstantiate):
        self.STRM.Elements(Streamname).Elements("Input").Elements(
            ""
        ).Value = AlwaysInstantiate
    ...
    def STRM_Set_AddOptWaterOnlyCheck(self, Streamname, WaterOnlyCheck):
        self.STRM.Elements(Streamname).Elements("Input").Elements(
            "CHECK_FREE_W"
        ).Value = WaterOnlyCheck

    def STRM_Set_AddOptRemoveMissingPhase(
        self, Streamname: str, YesOrNo: Literal["YES", "NO"]
    ) -> None:
        self.STRM.Elements(Streamname).Elements("Input").Elements(
            "AUTO_PHASE"
    ...
    def STRM_Set_AddOptFlashFormulation(
        self, Streamname: str, FlashFormulation: Literal["PML", "SMOOTHING"]
    ) -> None:  # Option is "PML", "SMOOTHING"
        self.STRM.Elements(Streamname).Elements("Input").Elements(
            "FLASH_FORM"
        ).Value = FlashFormulation
    ...
    def BLK_DSTWU_GET_OUTPUTS(
        ...
        # Page 1
        MinimumRefluxRatio = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("MIN_REFLUX").Value
        )
        ActualRefluxRatio = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("ACT_REFLUX").Value
        )
        MinimumNStage = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("MIN_STAGES").Value
        )
        ActualNStage = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("ACT_STAGES").Value
        )
        FeedStage = (
            self.BLK.Elements(Blockname)
            .Elements("Output")
            .Elements("FEED_LOCATN")
            .Value
        )
        ActualNumberOfStagesAboveFeed = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("RECT_STAGE").Value
        )
        ReboilerHeatingRequired = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("REB_DUTY").Value
        )
        CondenserCoolingRequired = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("COND_DUTY").Value
        )
        DestillateTemperature = (
            self.BLK.Elements(Blockname)
            .Elements("Output")
            .Elements("DISTIL_TEMP")
            .Value
        )
        BottomTemperature = (
            self.BLK.Elements(Blockname)
            .Elements("Output")
            .Elements("BOTTOM_TEMP")
            .Value
        )
        DestillateFeedFraction = (
            self.BLK.Elements(Blockname)
            .Elements("Output")
            .Elements("DIST_VS_FEED")
            .Value
        )
        ...
        MoleFlowBalanceIN = (
            self.BLK.Elements(Blockname)
            .Elements("Output")
            .Elements("BAL_MOLI_TFL")
            .Value
        )
        MoleFlowBalanceOUT = (
            self.BLK.Elements(Blockname)
            .Elements("Output")
            .Elements("BAL_MOLO_TFL")
            .Value
        )
        MoleFlowBalanceRelDifference = (
            self.BLK.Elements(Blockname)
            .Elements("Output")
            .Elements("BAL_MOLR_TFL")
            .Value
        )
        MassFlowBalanceIN = (
            self.BLK.Elements(Blockname)
            .Elements("Output")
            .Elements("BAL_MASI_TFL")
            .Value
        )
        MassFlowBalanceOUT = (
            self.BLK.Elements(Blockname)
            .Elements("Output")
            .Elements("BAL_MASO_TFL")
            .Value
        )
        MassFlowBalanceRelDifference = (
            self.BLK.Elements(Blockname)
            .Elements("Output")
            .Elements("BAL_MASR_TFL")
            .Value
        )
        EnthalpyBalanceIN = (
            self.BLK.Elements(Blockname)
            .Elements("Output")
            .Elements("TOT_ENTH_ABS")
            .Value
        )
        EnthalpyBalanceOUT = (
            self.BLK.Elements(Blockname)
            .Elements("Output")
            .Elements("BAL_ENTH_OUT")
            .Value
        )
        EnthalpyBalanceRelDifference = (
            self.BLK.Elements(Blockname)
            .Elements("Output")
            .Elements("TOT_ENTH_REL")
            .Value
        )
        # PAGE 3
        StagenumberList = []
        ...
        # PAGE 4
        ConvergenceStatus = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("BLKSTAT").Value
        )
        ConvergenceMessage = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("BLKMSG").Value
        )
        PropertyStatus = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("PROPSTAT").Value
        )
    ...
    def BLK_DSTWU_Get_EnthalpyBalanceIN(self, Blockname):
        return (
            self.BLK.Elements(Blockname).Elements("Output").Elements("TOT_ENTH").Value
        )
    ...
    def BLK_FLASH2_GET_OUTPUTS(
        ...
        # Page 1
        OutletTemperature = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("B_TEMP").Value
        )
        OutletPressure = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("B_PRES").Value
        )
        VaporFractionMole = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("B_VFRAC").Value
        )
        VaporFractionMass = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("MVFRAC").Value
        )
        HeatingDuty = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("QCALC").Value
        )
        NetDuty = self.BLK.Elements(Blockname).Elements("Output").Elements("QNET").Value
        FirstLiquidtoTotalLiquidRatio = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("LIQ_RATIO").Value
        )
        PressureDrop = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("PDROP").Value
        )
        ...
        # PAGE 3
        CompoundLister = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("F").Elements
        )
        ...
        for compound in CompoundLister:
            ...
            EquilibriumConstantList.append(
                self.BLK.Elements(Blockname)
                .Elements("Output")
                .Elements("B_K")
                .Elements(Compoundname)
    ...
    def BLK_RADFRAC_GET_OUTPUTS(
        ...
        Condenser_Temperature = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("TOP_TEMP").Value
        )
        Condenser_SubcooledTemp = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("SCTEMP").Value
        )
        Condenser_HeatingDuty = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("COND_DUTY").Value
        )
        Condenser_SubcooledDuty = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("SCDUTY").Value
        )
        Condenser_DistillateRate = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("MOLE_D").Value
        )
        Condenser_RefluxRate = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("MOLE_L1").Value
        )
        Condenser_RefluxRatio = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("MOLE_RR").Value
        )
        Condenser_FreeWaterDistillateRate = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("MOLE_DW").Value
        )
        Condenser_FreeWaterRefluxRatio = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("RW").Value
        )
        Condenser_DistillateToFeedRatio = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("MOLE_DFR").Value
        )
        ...
        Reboiler_BottomsRate = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("MOLE_B").Value
        )
        Reboiler_BoilupRate = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("MOLE_VN").Value
        )
        Reboiler_BoilupRatio = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("CMF_MAMX").Value
        )
        Reboiler_BottomsToFeedRatio = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("MOLE_BFR").Value
        )
        ...
        for compound in CompoundLister:
            ...
            try:
                ReboilerMoleFractionInLiquidList.append(
                    self.BLK.Elements(Blockname)
                    .Elements("Output")
                    .Elements("TH_X")
                    .Elements(Compoundname)
                ...
                ReboilerMoleFractionInVaporList.append(
                    self.BLK.Elements(Blockname)
                    .Elements("Output")
                    .Elements("TH_Y")
                    .Elements(Compoundname)
            ...
            CompoundNameList.append(Compoundname)
        try:
            Thermosiphon_Pressure = (
                self.BLK.Elements(Blockname)
                .Elements("Output")
                .Elements("TH_PRES_OUT")
                .Value
            )
            Thermosiphon_Temperature = (
                self.BLK.Elements(Blockname)
                .Elements("Output")
                .Elements("TH_TEMP_OUT")
                .Value
            )
            Thermosiphon_MolarVaporFraction = (
                self.BLK.Elements(Blockname)
                .Elements("Output")
                .Elements("TH_VFRAC_OUT")
                .Value
            )
            Thermosiphon_MolarFlow = (
                self.BLK.Elements(Blockname)
                .Elements("Output")
                .Elements("TH_MOLEFLOW")
                .Value
            )
            Thermosiphon_MassFlow = (
                self.BLK.Elements(Blockname)
                .Elements("Output")
                .Elements("TH_MASSFLOW")
                .Value
            )
            Thermosiphon_HeatDuty = (
                self.BLK.Elements(Blockname)
                .Elements("Output")
                .Elements("TH_DUTY")
                .Value
            )
            Thermosiphon_FirstliquidByTotalLiquidRatio = (
                self.BLK.Elements(Blockname)
                .Elements("Output")
                .Elements("LIQ_RATIO")
                .Value
        ...
        Dictionary = {
            ...
            "Condenser_RefluxRatio": Condenser_RefluxRatio,
    ...
    def BLK_RADFRAC_Get_SplitFraction(self, Blockname, Compoundname, OutputStreamname):
        return (
            ...
            .Elements("MASS_CONC")
            .Elements(Compoundname)
            .Elements(OutputStreamname)
            .Value
        )

    def BLK_RADFRAC_Get_SplitFraction_List(
        self, Blockname: str, OutputStreamName: str
    ) -> list:
        CompoundLister = (
            self.BLK.Elements(Blockname)
            .Elements("Output")
    ...
    def BLK_RCSTR_GET_OUTPUTS(
        ...
        NetHeatDuty = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("QNET").Value
        )
        ReactorVolume = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("TOT_VOL").Value
        )
        VaporPhaseVolume = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("VAP_VOL").Value
        )
        LiquidPhaseVolume = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("LIQ_VOL").Value
        )
        Liquid1PhaseVolume = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("LIQ1_VOL").Value
        )
        SaltPhaseVolume = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("SALT_VOL").Value
        )
        CondensedPhaseVolume = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("COND_VOL").Value
        )
        ReactorResidenceTime = (
            self.BLK.Elements(Blockname)
            .Elements("Output")
            .Elements("TOT_RES_TIME")
            .Value
        )
        VaporPhaseResidenceTime = (
            self.BLK.Elements(Blockname)
            .Elements("Output")
            .Elements("VAP_RES_TIME")
            .Value
        )
        CondensedPhaseResidenceTime = (
            self.BLK.Elements(Blockname)
            .Elements("Output")
            .Elements("COND_RES_TIM")
            .Value
        )
        ...
        MoleFlowBalanceGenerated = (
            self.BLK.Elements(Blockname)
            .Elements("Output")
            .Elements("BAL_MOLG_TFL")
            .Value
        )
        MassFlowBalanceIN = (
            self.BLK.Elements(Blockname)
        ...
        MassFlowBalanceGenerated = (
            self.BLK.Elements(Blockname)
            .Elements("Output")
            .Elements("BAL_MASG_TFL")
            .Value
        )
        EnthalpyBalanceIN = (
            self.BLK.Elements(Blockname)
        ...
        EnthalpyBalanceGenerated = (
            self.BLK.Elements(Blockname)
            .Elements("Output")
            .Elements("BAL_ENTH_GEN")
            .Value
        )
    ...
    def BLK_RPLUG_GET_OUTPUTS(
        ...
        MinimumReactorTemperature = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("TMIN").Value
        )
        MaximumReactorTemperature = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("TMAX").Value
        )
        ResidenceTime = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("RES_TIME").Value
        )
        ThermalFluidInletTemperature = (
            self.BLK.Elements(Blockname)
            .Elements("Output")
            .Elements("COOLANT_TIN")
            .Value
        )
        ThermalFluidInletVaporFraction = (
            self.BLK.Elements(Blockname)
            .Elements("Output")
            .Elements("COOLANT_VIN")
            .Value
        )
    ...
    def BLK_RYIELD_GET_OUTPUTS(
        ...
        FirstLiquidbyTotalLiquidFraction = (
            self.BLK.Elements(Blockname).Elements("Output").Elements("LIQ_RATIO").Value
        )
        # Page 2
        MoleFlowBalanceIN = (
    ...
    def BLK_FSPLITTER_GET_OUTPUTS(
        ...
        for compound in SplitFractionElementList:
            ...
            StreamOrderList.append(
                self.BLK.Elements(Blockname)
                .Elements("Output")
                .Elements("STREAM_ORDER")
                .Elements(Compoundname)
                .Value
            )
            CompoundNameList.append(Compoundname)
    ...
    def CAL_Column_Diameter(
        self, pressure, n_stages, vapor_flows, stage_mw, stage_temp
    ):
        P = pressure
        f = float(1.6)
        R = float(8.314)
    ...
    def CAL_LMTD(self, tops_temperature):
        ...
        delta_Tm_cnd = (
            (tops_temperature - T_cool_in)
            * (tops_temperature - T_cool_out)
            * ((tops_temperature - T_cool_in) + (tops_temperature - T_cool_out))
            / 2
        ) ** (1 / 3)
    ...
    def CAL_InvestmentCost(
        ...
        D = self.CAL_Column_Diameter(
            pressure, n_stages, vapor_flows, stage_mw, stage_temp
        )  # Column diameter [m]
        A_cnd = self.CAL_HT_Condenser_Area(
            condenser_duty, tops_temperature
        )  # Heat transfer area of condenser [m2]
        A_rbl = self.CAL_HT_Reboiler_Area(
            reboiler_temperature, reboiler_duty
        ...
        F_htx_d = (
            0.8  # Correction factor for design type: fixed-tube sheet [-] (0.8, fixed)
        )
    ...
    def CAL_OperatingCost(self, reboiler_duty, condenser_duty):
        ...
        delta_hv = (
            34794  # Molar heat of condensation of 16 bar steam [J/mol] (34794, fixed)
        )
        ...
        C_op_cnd = (
            condenser_duty / 1000000 * c_cw * 3600 / (c_p * (T_cool_out - T_cool_in))
        )  # €/h
    ...
    def CAL_Annual_OperatingCost(self, reboiler_duty, condenser_duty):
        t_a = 8400
        OperatingCost = (
            self.CAL_OperatingCost(reboiler_duty, condenser_duty) * t_a / 1000
        )
        return OperatingCost

    def CAL_stream_value(
        self, MoleFlowList, product_specification=0.95
        ...
        is_purity, component_purities = self.CAL_purity_check(
            MoleFlowList, product_specification
        )
        ...
        for entry in component_specifications:
            if sum(is_purity) > 0:
                component_specifications[entry]["mass flow"] = (
                    MoleFlowList[component_specifications[entry]["index"]]
                    * component_specifications[entry]["molar weight"]
                    / 1000
                    * up_time
                )  # ton/year
                component_specifications[entry]["stream value"] = (
                    is_purity[component_specifications[entry]["index"]]
        ...
        total_stream_value = sum(
            d["stream value"] for d in component_specifications.values() if d
        )
