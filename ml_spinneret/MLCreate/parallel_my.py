...
def partial_load(index, data_path):
    return load_data(index, data_path)

def parallel_loader(data_path, num_workers=6):
    with Pool(num_workers) as pool:
        data = pool.starmap(partial_load, [(i, data_path) for i in range(num_workers)])
...
class MLmodule:
    ...
    def load_dataset(self):
        self.dataset = parallel_loader(self.dataset_filename, num_workers=self.num_workers)
    ...
    def fit_model(self, outer_values, central_values):
        train_set, test_set = self.data_preprocessing(outer_values, central_values)
        self.logger.info('Обучение сети')
        # self.model = nn.DataParallel(self.model)
        for epoch in range(self.nn_params['num_epochs']):
            for batch_inp, batch_out in tqdm(train_set):
                self.model.train()
            ...
            self.logger.info(f'Epoch: {epoch}, Loss: {loss.item() * 100:5f}')

        self.logger.info(f'Тестирование ...')
        final_loss = []
        self.model.eval()
        for batch_inp, batch_out in tqdm(test_set):
            outputs = self.model(batch_inp)
            loss = self.criterion(outputs, batch_out)
    ...
    def action(self):
        ...
        self.fit_model(all_outer_values, all_central_values)
...
if __name__ == '__main__':
    ...
    module = MLmodule(params={'MODEL_NAME': 'MODEL_V_6.pth', 'num_workers': 4},
                    nn_params=nn_params,
